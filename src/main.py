...
def infer(model: Model, fn_img: Path) -> None:
    ...
    os.system('cls')
    ## function call for fuzzywuzzy
    tablet, link = matching(recognized[0])
    ## function for selenium to fetch cheap alternative of the medicine
    name, price, alt_link = webList(tablet)
    print(f'Recognized: "{recognized[0]}"')
    print(f'Probability: {probability[0]}')
    print(f'Medicine: "{tablet}"')
    print(f'Link: "{link}"')
    print(f'Alternative: "{name}"')
    print(f'Price: "{price}"')
    print(f'Alternative Link: "{alt_link}"')

    with open('Output.txt', 'a') as op:
        op.write('Alternative Name: ' + name + '\n' 
            + 'Price: ' + price +'\n' 
            + 'Alternative Link: ' + alt_link + '\n\n\n')
...
def main():
    ...
    parser.add_argument('--img_file', help='Image used for inference.', type=Path, default='../data/brufen.png')
